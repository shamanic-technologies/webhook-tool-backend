...
export const createWebhookController = async (req: AuthenticatedRequest, res: Response<ServiceResponse<Webhook>>, next: NextFunction) => {
    ...
    try {
        ...
        const webhookData = {
            ...
            clientUserIdentificationMapping: validationResult.data.clientUserIdentificationMapping as Record<UtilitySecretType, string>,
            conversationIdIdentificationMapping: validationResult.data.conversationIdIdentificationMapping,
        };
        ...
        for (const secretTypeNeededForMapping in webhookData.clientUserIdentificationMapping) {
            if (!webhookData.requiredSecrets.includes(secretTypeNeededForMapping as UtilitySecretType)) {
