...
function isUtilityInputSecret(secret: UtilitySecretType): secret is UtilityInputSecret {
    return Object.values(UtilityInputSecret).includes(secret as UtilityInputSecret);
}
...
function _validateLinkUserRequest(req: AuthenticatedRequest): LinkUserValidationResult {
    const paramsValidation = WebhookIdParamsSchema.safeParse(req.params);
    if (!paramsValidation.success) {
        return { errorResponse: formatValidationError(paramsValidation.error) } as LinkUserValidationResult;
    }
    const { webhookId } = paramsValidation.data;

    const clientUserId = req.serviceCredentials?.clientUserId;
    if (!clientUserId) {
        return { errorResponse: { success: false, error: 'Unauthorized', message: 'Client User ID header is required.' } } as LinkUserValidationResult;
    }
    const platformUserId = req.serviceCredentials?.platformUserId;
    if (!platformUserId) {
        return { errorResponse: { success: false, error: 'Unauthorized', message: 'Platform User ID missing.' } } as LinkUserValidationResult;
    }
...
async function _checkWebhookSetupStatus(
    ...
    const confirmationSecretDbType = UtilityActionConfirmation.WEBHOOK_URL_INPUTED;
    ...
    const isConfirmed = confirmationCheck.success && confirmationCheck.data.value === 'true';
    ...
    const requiredSecretsForIdentificationValues = Object.keys(webhook.clientUserIdentificationMapping) as UtilityInputSecret[];
    console.log('DEBUG: Required Identification Secret Values:', requiredSecretsForIdentificationValues);
    ...
    for (const secretTypeValue of allRequiredSecretValues) { 
        ...
        const gsmCheck = await checkSecretExistsGsm(UserType.Client, clientUserId, webhook.webhookProviderId, secretTypeValue);
        ...
        // Check if this secret VALUE is needed for identification
        } else if (requiredSecretsForIdentificationValues.includes(secretTypeValue as UtilityInputSecret)) {
            console.log(`DEBUG: Secret value ${secretTypeValue} exists and is needed for identification. Attempting to get its value...`);
            const secretValueResult = await getSecretGsm(UserType.Client, clientUserId, webhook.webhookProviderId, secretTypeValue);
            if (secretValueResult.success && secretValueResult.data.value !== null) {
                ...
                console.error(`Failed to get value for required secret ${secretTypeValue}, treating as missing.`);
    ...
    if (missingInputs.length > 0 || missingConfirmations.length > 0) {
        const setupNeededData: SetupNeeded = {
            needsSetup: true, utilityProvider: webhook.webhookProviderId,
...
export const linkUserController = async (
    ...
    try {
        ...
        if (isNewLink) {
             userWebhookRecord = await createUserWebhookService(webhookId, clientUserId, platformUserId, WebhookStatus.PENDING, null);
        }
        let currentStatus = userWebhookRecord!.status;
        ...
        if (setupStatus.isSetupNeeded) {
            if (currentStatus === WebhookStatus.ACTIVE) {
                await updateUserWebhookStatusService(webhookId, clientUserId, WebhookStatus.PENDING, null);
            }
            const response: SuccessResponse<SetupNeeded> = { success: true, data: setupStatus.setupNeededData! };
            return res.status(200).json(response);
        } else {
            // Check identifier count (moved logic)
            const requiredSecretsForIdentificationValues = Object.keys(webhook.clientUserIdentificationMapping) as UtilityInputSecret[];
            if (Object.keys(setupStatus.identifierValues!).length !== requiredSecretsForIdentificationValues.length) {
                 console.error("Mismatch count: required identification values vs fetched values.");
                 return res.status(500).json({ success: false, error: 'Internal Server Error', message: 'Failed to retrieve all required identification values.' });
            }

            if (!appConfig.hmacKey) {
                 throw new Error("HMAC key configuration error."); 
             }
             const identificationHash = computeIdentifierHash(setupStatus.identifierValues!, appConfig.hmacKey);
             userWebhookRecord = await updateUserWebhookStatusService(webhookId, clientUserId, WebhookStatus.ACTIVE, identificationHash);
